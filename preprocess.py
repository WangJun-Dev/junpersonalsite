...
def extract_links(content):
    # Match both [[wiki-style]] and [markdown-style](links)
    wiki_pattern = r'\[\[(.*?)\]\]'
    markdown_pattern = r'\[([^\]]+)\]\(([^)]+)\)'
    ...
    links.extend([link.split('|')[0] if '|' in link else link for link in wiki_links])
    
    # Extract markdown-style links
    markdown_links = re.findall(markdown_pattern, content)
    links.extend([link[1] for link in markdown_links if not link[1].startswith('http')])
    
    return links

def generate_indices(source_path):
    link_index = {
        "index": {
            "links": {},
            "backlinks": {},
    ...
    for root, _, files in os.walk(source_path):
        for file in files:
            if file.endswith('.md'):
                ...
                # Add to link index
                if file_links:
                    link_index["index"]["links"][clean_path] = file_links
                    for link in file_links:
                        clean_link = '/' + link.replace('.md', '')
                        if clean_link not in link_index["index"]["backlinks"]:
                            link_index["index"]["backlinks"][clean_link] = []
                        link_index["index"]["backlinks"][clean_link].append(clean_path)
                        
                        # Add to flat links list
                        link_index["links"].append({
                            "source": clean_path,
                            "target": clean_link
                        })
                
                # Add to content index
                content_index[clean_path] = {
    ...
    with open("assets/indices/linkIndex.json", 'w', encoding='utf-8') as f:
        json.dump(link_index, f, ensure_ascii=False, indent=2)
        
    with open("assets/indices/contentIndex.json", 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
  ...
  generate_indices(args.source_path)